...
const turn = (
  ...
  const nextIndex = (i => (i + 1 < players.length ? i + 1 : 0))(
    table.turnIndex
  );
  const props = {
    turnIndex: nextIndex,
    ...
    roundCount: table.turnIndex === 0 ? table.roundCount + 1 : table.roundCount,
  };
  ...
  if (newPlayer.outTurns > OUT_TURN_COUNT_ELIMINATION) {
    ...
    props.turnIndex = (i => (i + 1 < players_.length ? i + 1 : 0))(
      props.turnIndex
    );
...
const removePlayer = (
  players: ReadonlyArray<Player>,
  lands: ReadonlyArray<Land>
) => (player: Player): [ReadonlyArray<Player>, ReadonlyArray<Land>] => {
  return [
